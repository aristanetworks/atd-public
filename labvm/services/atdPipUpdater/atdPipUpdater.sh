...
PIP_INST=$(cat serviceUpdater.yml | shyaml get-value pipPackages.Install)
PIP_REM=$(cat serviceUpdater.yml | shyaml get-value pipPackages.Remove)

# Evaluate if there are items within the list
if [ "$PIP_INST" != "None" ]
then
    # Grab full list and install all packages
    N_PIP_PKGS=$(cat serviceUpdater.yml | shyaml get-values pipPackages.Install)
...
then
    ...
    R_PIP_PKGS=$(cat serviceUpdater.yml | shyaml get-values pipPackages.Remove)
